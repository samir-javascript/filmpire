...
import { useDispatch } from 'react-redux';
import { searchMovie } from '../../features/CurrentGenreOrCategory';
import { SearchContainer, SearchDiv, StackStyles } from './styles';
...
export default function FreeSolo() {
  ...
  return (
    <SearchContainer spacing={2} sx={{ width: 250 }}>
      <Autocomplete
        id="free-solo-demo"
        freeSolo
        value={query}
        onChange={(event, newValue) => setQuery(newValue)}
        options={top100Films.map((option) => (typeof option.title === 'string' ? option.title : option.title.toString()))}
        renderInput={(params) => (
          <TextField
            {...params}
            ...
            sx={isSmallScreen ? { mt: '-15px', border: "none !important", hight: '5px' } : {}}
          />
        )}
      />
    </SearchContainer>
